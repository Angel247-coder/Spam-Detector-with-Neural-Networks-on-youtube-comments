"""
IA YouTube Spam Detector — v8.0  "Full Dataset"
...
SPAM (modelo de clasificación)
  ...
  Nota: 45k es el 45K Rows dataset (2025) — cubre 66 canales
  multilingüe, muy superior al dataset de música de 2012.

SENTIMIENTO (modelo de clasificación)
  3. youtube_comments_cleaned.csv  1 032 225 filas  CommentText / Sentiment
     → muestra balanceada de 80 000 filas por clase (240k total)
  4. YouTube Comments Dataset …csv  45 005 filas  comment_text / label_sentiment
  ──────────────────────────────────────────────
  Total combinado: ~285 000 filas  │  Positive / Neutral / Negative
  Modelo: LR(C=1, balanced, saga) + TF-IDF bigramas 30k features
  Accuracy holdout 15%: ~66.5%  (VADER en los mismos datos: ~50%)
...
class SpamFeatures(BaseEstimator, TransformerMixin):
    """
    9 features numéricas basadas en el EDA del documento de proyecto.
    Correlaciones con CLASS del análisis §3.1:
      palabras_spam (r=0.69), longitud_chars (r=0.34), contiene_url (r=0.33),
      ratio_mayus (r=0.05), exclamaciones (r=0.01).
    longitud_palabras eliminada (colineal con longitud_chars, r=0.91, §4.2).
    """
...
# ─────────────────────────────────────────────────────────────────
# CARGA Y COMBINACIÓN DE LOS 4 DATASETS
# ─────────────────────────────────────────────────────────────────
RUTAS = {
    "spam_clasico":    "Youtube-Spam-Dataset.csv",
    "spam_45k":        "YouTube Comments Dataset with Sentiment Toxicity and Spam Labels (45K Rows).csv",
    "sentiment_1m":    "youtube_comments_cleaned.csv",
    "export_anterior": "2026-05-13T17-45_export.csv",
}
...
def cargar_datos_spam() -> pd.DataFrame:
    """
    Combina todas las fuentes disponibles con etiqueta de spam.
    Fuentes reconocidas:
      • Youtube-Spam-Dataset.csv        → CONTENT / CLASS
      • 45K dataset                     → comment_text / label_spam
      • export anterior                 → Comentario / Spam
    """
    partes = []

    # Fuente 1: dataset clásico (1 956 filas)
    if os.path.exists(RUTAS["spam_clasico"]):
    ...
    if os.path.exists(RUTAS["spam_45k"]):
    ...
    if os.path.exists(RUTAS["export_anterior"]):
...
def cargar_datos_sentimiento() -> pd.DataFrame:
    """
    Combina las fuentes con etiqueta de sentimiento.
    Muestra 80 000 por clase del dataset de 1M para mantener balance.
    """
    partes = []

    # Fuente principal: 1M filas (muestreamos 80k/clase)
    if os.path.exists(RUTAS["sentiment_1m"]):
        df = pd.read_csv(RUTAS["sentiment_1m"], usecols=["CommentText", "Sentiment"])
        ...
        for lbl in ["positive", "neutral", "negative"]:
            ...
            muestras.append(sub.sample(n=min(80_000, len(sub)), random_state=42))
        partes.append(pd.concat(muestras, ignore_index=True))

    # Fuente complementaria: 45k dataset (sentimientos recientes, multilingüe)
    if os.path.exists(RUTAS["spam_45k"]):
    ...
    if not partes:
        ...
        raise FileNotFoundError("No se encontró ningún dataset de sentimiento.")
...
def entrenar_spam(df: pd.DataFrame):
    ...
    # sample_weight para compensar el desbalance 1:25
...
def analizar(
    ...
    spam_pipe,
    sent_pipe,
    batch_spam: bool = False,
) -> dict:
    texto = str(texto or "").strip()
    if not texto:
        return {"spam": 0, "spam_conf": 0.0,
                "sentimiento": "Neutral", "sent_conf": 50.0, "motivo": ""}
...
def descargar_comentarios(api_key: str, video_id: str, limite: int) -> list[dict]:
    ...
    while len(comentarios) < limite:
        ...
        kw = dict(part="snippet", videoId=video_id,
                  maxResults=por_pagina, order="time", textFormat="plainText")
        if page_token:
...
def resultados_a_df(filas: list[dict]) -> pd.DataFrame:
...
def mostrar_resultados(df_res: pd.DataFrame):
    ...
    k2.metric("Spam",         f"{(df_res['Spam']=='🚨 SÍ').mean()*100:.1f}%",
              delta_color="inverse")
    ...
    if not spam_df.empty:
        ...
        m.columns = ["Motivo", "N"]
        st.plotly_chart(px.bar(m, x="Motivo", y="N",
                               title="Motivo de clasificación como spam"),
                        use_container_width=True)

    st.subheader("📋 Tabla detallada")
    st.info("🔒 'Seudónimo' = SHA-256. Ningún nombre real en esta tabla ni en el CSV.")
    st.dataframe(df_res, use_container_width=True)
    st.download_button("📥 Descargar CSV anonimizado",
                       df_res.to_csv(index=False).encode("utf-8"),
                       "auditoria_anonimizada.csv", "text/csv")
...
def main():
    ...
    # Entrenar modelos
    with st.spinner("Entrenando modelos (primera carga ~60 s)…"):
    ...
    with st.sidebar:
        ...
        if opcion == "🎬 Auditoría en tiempo real":
            ...
            api_key = st.text_input("API Key", type="password",
                                    help="Google Cloud Console → APIs → YouTube Data API v3")
    ...
    # ════════════════════════════════════════════════════════════
        ...
        st.markdown(
            '<div class="rgpd-box">🔒 <b>RGPD:</b> Autores seudonimizados (SHA-256) '
            'antes de cualquier procesado. CSV exportado sin nombres reales.</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            '<div class="warn-box">⚠️ <b>ToS YouTube:</b> Analiza CSV con comentarios '
            'ya descargados. Sin conexión a APIs externas.</div>',
            unsafe_allow_html=True,
        )
        upload = st.file_uploader(
            "CSV con columna **Comentario**, **content**, **text** o **comment**",
            type=["csv"],
        )
    ...
    # ════════════════════════════════════════════════════════════
        ...
        st.markdown(
            '<div class="rgpd-box">🔒 <b>RGPD:</b> Autores seudonimizados (SHA-256). '
            'Sin persistencia. CSV anonimizado.</div>',
            unsafe_allow_html=True,
        )
        ...
        if st.button("🚀 Analizar vídeo", type="primary"):
            ...
            with st.spinner("Descargando vía YouTube Data API v3…"):
                try:
                    ...
                    st.info("Comprueba la API Key, que YouTube Data API v3 esté "
                            "habilitada y que no hayas superado la cuota (10k unidades/día).")
                    return
    ...
    # ════════════════════════════════════════════════════════════
        ...
        with tab_spam:
            st.markdown(
                f"Entrenado con **{m_spam['n_train']:,}** muestras "
                f"({m_spam['n_spam']:,} spam · {m_spam['n_real']:,} reales). "
                "Holdout estratificado 20%."
            )
            ...
            with col_cm:
                st.pyplot(plot_cm(m_spam["cm"], ["Real", "Spam"],
                                  "Confusión spam (20% holdout)"))
            with col_rep:
...
| Datos spam | 45K dataset (2025) + dataset clásico |
        ...
        with tab_sent:
            st.markdown(
                ...
                "(80k/clase de 1M dataset + 45K dataset). Holdout 15%."
            )
            c1, c2 = st.columns(2)
            ...
            col_cm, col_rep = st.columns([1, 1.5])
            with col_cm:
                st.pyplot(plot_cm(m_sent["cm"],
                                  ["positive", "neutral", "negative"],
                                  "Confusión sentimiento (15% holdout)"))
            with col_rep:
                st.code(m_sent["reporte"])

            st.markdown(
                '<div class="good-box">ℹ️ 66.5% de accuracy en clasificación de sentimiento '
                '3 clases sobre 285k comentarios reales es el techo realista para TF-IDF. '
                'Supera a VADER (~50%) en este corpus multilingüe.</div>',
                unsafe_allow_html=True,
            )
            st.divider()
            st.markdown("""
...
| Solver | SAGA (rápido en datasets grandes) |
...
| Datos | 240k (1M dataset) + 45k dataset = 285k |
            """)
    ...
    # ════════════════════════════════════════════════════════════
        ...
        with tab_s:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.plotly_chart(px.pie(df_s, names="Etiqueta",
                                       title="Balance Real / Spam", hole=0.4,
                                       color="Etiqueta",
                                       color_discrete_map={"Real":"#2ecc71","Spam":"#e74c3c"}),
                                use_container_width=True)
            with c2:
                df_s["longitud"] = df_s["text"].str.len()
                st.plotly_chart(px.box(df_s, x="Etiqueta", y="longitud",
        ...
        with tab_sent2:
            ...
            st.plotly_chart(px.pie(df_sv, names="Etiqueta",
                                   title="Distribución de sentimiento en training",
                                   hole=0.4,
                                   color="Etiqueta",
                                   color_discrete_map={
                                       "Positive":"#2ecc71",
    ...
    st.divider()
    st.caption(
        "v8.0 · MLP spam (47k) · LR sentimiento (285k) · "
        "YouTube Data API v3 · RGPD Art. 25"
    )
